    ...
    if showAttr and attStr and #attStr > 0 then
        ...
        for i, v in ipairs(attStr) do
            ...
            if v.str then
                local attr = v.str
                if  job ~= 0 then
                    if string.find(attr, config[job][1]) ~= nil then
                        ----SL:Print(attr ,  job)
                        attr = string.gsub(attr, "%)", ") <font color='#f1f900'>"..config[job][2].."专属 </font>")
                        ----SL:Print("xxxsxs----",attr)
                    end
                end
                rich_att_base = GUI:RichText_Create(contentPanel, "rich_att_base_" .. i, 0, 0, attr, richWidth, _setFontSize or _DefaultFSize, "#FFFFFF", vspace, nil, fontPath)
