...
function Bag.main(page)
    ...
    Bag.Table.Panel = Bag._ui["Panel_1"]
    ---if not  isWin32 then
        local x = 0
        local y = 0
        local font = 16
        for i=1,#Bag.Table do
            if isWin32 then
                x = math.floor((i-1)/3)*140 - 56 + 131
                y = -16 - (i-1)%3*29 + 120
            else
                x = 70 + math.floor((i-1)/3)*115 + 10 + 27 - 38 + 15
                y = 70 - (i-1)%3*29 - 2 + 34 + 10 + 20
            end
            local item_data = {}
            item_data.index = Bag.Table[i][2]
            item_data.look  = true
            ...
            local _TextHandle = GUI:Text_Create(Bag.Table.Panel, "money"..i, x + 10, y-38, font, "#f6ff00", ""..(SL:GetMetaValue("MONEY",  1) + SL:GetMetaValue("MONEY",  3)))
            if _TextHandle then
                GUI:setContentSize(_TextHandle, {width = 220, height = 24 })
                GUI:Text_enableOutline(_TextHandle, "#000000", 1)
                GUI:setAnchorPoint(_TextHandle, 0.00, 0.00)
                GUI:setTouchEnabled(_TextHandle, false)
                GUI:setTag(_TextHandle, 0)
            end
        end
    ---end

    local _Handle = GUI:GetWindow(Bag.Table.Panel,"Button_1")
    if _Handle then
        GUI:addOnClickEvent(_Handle, function()
            SL:SubmitForm("基础功能通用_openWindow",4)
        end)
    end

    _Handle = GUI:GetWindow(Bag.Table.Panel,"Button_2")
    if _Handle then
        GUI:addOnClickEvent(_Handle, function()
            SL:SubmitForm("基础功能通用_openWindow",1)
        end)
    end

    SL:SubmitForm("安全箱_main")
    -- 初始化左侧背包页签
    Bag.InitPage()
...
function Bag.OnUpdateGold(data)
    ...
    for i=1,#Bag.Table do
        local _Handle = GUI:GetWindow(Bag.Table.Panel,"money"..i)
        if _Handle then
            GUI:Text_setString(_Handle, "".. (SL:GetMetaValue("MONEY",  1) + SL:GetMetaValue("MONEY",  3)))
        end
    end
end
