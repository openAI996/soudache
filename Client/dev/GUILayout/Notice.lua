...
function Notice.OnShowSystemTips(str)
    ...
    --if string.find(str,"网住了") then
    --    return
    --end
    --if str == "承受状态中，无法使用" then
    --    return
    --end
    -- node
...
function Notice.OnShowItemTips(info)
    ...
    if string.find(info.str,"隐藏装备") ~= nil and string.find(info.str,"隐藏装备") ~= "" then
    else
    ...
    local function showTips()
        ...
        str = string.gsub(str,"获得","+")
        str = string.gsub(str,"消耗","-")
        ...
        if Notice.isPC then 
            ...
            str1 = string.gsub(str1,"-","")
            local tab_str1 = split_string(str1, "*")
            local tab_str2 = split_string(str, " ")
            
            local IMG = GUI:Image_Create(node, "IMG", -35, -9, "res/public/111111111.png")
            local setData  = {}
            setData.index = SL:GetMetaValue("ITEM_INDEX_BY_NAME", tab_str2[2])  -- 物品Index
            setData.look  = false                  -- 是否显示tips
            setData.bgVisible = false              -- 是否显示背景框
            setData.count = (string.gsub(tab_str1[2], "^%s*(.-)%s*$", "%1") )                     -- 物品数量
            setData.noMouseTips = true          -- 鼠标移入不显示tips

            local ItemShow = GUI:ItemShow_Create(node, "Item", -29, -3, setData)
            GUI:setScale(ItemShow, 0.6)
            local text = GUI:BmpText_Create(node, "text", 0, 0, SL:ConvertColorFromHexString(data.color), str,"fonts/font4.fnt")
            ...
            str2 = string.gsub(str2,"-","")
            local tab_str2 = split_string(str2, "*")
            local tab_str1_2 = split_string(tab_str2[1], ">")
            local tab_str1_3 = split_string(tab_str2[2], "<")
            
            local tab_str2 = split_string(str, " ")

            local IMG = GUI:Image_Create(node, "IMG", -35, -16, "res/public/111111111.png")
            local setData  = {}
            setData.index = SL:GetMetaValue("ITEM_INDEX_BY_NAME", tab_str2[6])  -- 物品Index
            setData.look  = false                  -- 是否显示tips
            setData.bgVisible = false              -- 是否显示背景框
            setData.count = (string.gsub(tab_str1_3[1], "^%s*(.-)%s*$", "%1") )                  -- 物品数量
            setData.noMouseTips = true          -- 鼠标移入不显示tips
            local ItemShow = GUI:ItemShow_Create(node, "Item", -39, -19, setData)
        ...
        if Notice.isPC then 
            -- 最多6条
            while #Notice._itemTipsCells > 6 do
        ...
        local iconMoveY = SL:GetMetaValue("WINPLAYMODE") and 0 or -30

        local actionTag = 999
        for i = 1, #Notice._itemTipsCells do
            if Notice._itemTipsCells[i] then 
                GUI:setPositionY(Notice._itemTipsCells[i], (size.height+13) * (#Notice._itemTipsCells - i - 0.5))
                local action  = GUI:ActionMoveTo(0.15, 0, iconMoveY+(size.height+13) * (#Notice._itemTipsCells - i + 0.5))
                GUI:setTag(action, actionTag)
...
function Notice.CheckNoticeAttribute()
    ...
    for i, v in ipairs(showAttrs) do
        ...
        data.name = v.name
        data.attr = v.value
        if string.find(data.name,"是否开启护身") ~= nil and string.find(data.name,"是否开启护身") ~= "" then
