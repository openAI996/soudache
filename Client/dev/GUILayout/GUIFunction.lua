...
function GUIFunction:GenerateChatMiniItem(data)
    if string.find(data.Msg,"你被网住了") then
        ---SL:Print("xxxx",data.Msg)
        return
    end
    local CHANNEL   = SL:GetMetaValue("CHAT_CHANNEL_ENUM")
...
-- BUFF人物缩放时buff位置偏移
function GUIFunction:GetHUDTopOffset(actorID, hudType, hudIndex)
    -- local offset = SL:GetMetaValue("BUFF_SACLE_OFFSET", actorID)
    return 0, 0
end

function GUIFunction:GetBuffAvatarSacleOffset(actorID, buffID)
    -- local offset = SL:GetMetaValue("BUFF_SACLE_OFFSET", actorID)
    return 0,0
end

-- 按住Shift键鼠标左键点击检测
function GUIFunction:CheckShiftTarget(actorID)
    return false
end